...
def exceptPrint(e, excinfo, srcinfo):
    ...
    if gglobs.redirect:
        ...
        dprint("EXCEPTION: Traceback:\n", traceback.format_exc(), end='', debug=True) # more extensive info
    print(NORMALCOLOR)
...
def readGeigerLogConfig():
    ...
    # is it error free?
        try:
            ...
            exceptPrint(e, sys.exc_info(), msg)
    ...
    # Defaults
        ...
        t = getConfigEntry("Defaults", "DefaultCalib1st", "str" )
        if t != "WARNING":
            try:                                gglobs.DefaultCalib1st = float(t)
            except:                             gglobs.DefaultCalib1st = 154
        if gglobs.DefaultCalib1st <= 0:         gglobs.DefaultCalib1st = 154
        dprint(infostr.format("DefaultCalib1st", gglobs.DefaultCalib1st))

        # calibration 2nd tube is 2.08 CPM/(µSv/h), =  0.48 in units of µSv/h/CPM
        t = getConfigEntry("Defaults", "DefaultCalib2nd", "str" )
        if t != "WARNING":
            try:                                gglobs.DefaultCalib2nd = float(t)
            except:                             gglobs.DefaultCalib2nd = 2.08
        if gglobs.DefaultCalib2nd <= 0:         gglobs.DefaultCalib2nd = 2.08
        dprint(infostr.format("DefaultCalib2nd", gglobs.DefaultCalib2nd))

        # calibration 3rd tube is 154 CPM/(µSv/h), =  0.0065 in units of µSv/h/CPM
        t = getConfigEntry("Defaults", "DefaultCalib3rd", "str" )
        if t != "WARNING":
            try:                                gglobs.DefaultCalib3rd = float(t)
            except:                             gglobs.DefaultCalib3rd = 154
        if gglobs.DefaultCalib3rd <= 0:         gglobs.DefaultCalib3rd = 154
        dprint(infostr.format("DefaultCalib3rd", gglobs.DefaultCalib3rd))
    ...
    # GMCDevice
        ...
        if gglobs.GMCActivation: # show the other stuff only if activated
            ...
            t = getConfigEntry("GMCDevice", "GMClocationBug", "str" )
            if t != "WARNING":
                if t.upper() == "AUTO":     gglobs.GMClocationBug = "auto"
                else:                       gglobs.GMClocationBug = t
                dprint(infostr.format("Location Bug", gglobs.GMClocationBug))
...
############
                dprint(infostr.format("GMC_SPIRpage", gglobs.GMC_SPIRpage))
            ...
            if t != "WARNING":
                ...
                dprint(infostr.format("GMC_SPIRbugfix", gglobs.GMC_SPIRbugfix))

            # GMCDevice GMCconfigsize
            t = getConfigEntry("GMCDevice", "GMCconfigsize", "upper" )
            if t != "WARNING":
                t = config.get("GMCDevice", "GMCconfigsize")
                if   t == '256':  gglobs.GMCconfigsize = 256
                elif t == '512':  gglobs.GMCconfigsize = 512
                else:             gglobs.GMCconfigsize = 'auto'
                dprint(infostr.format("GMCconfigsize", gglobs.GMCconfigsize))
            ...
            if t != "WARNING":
                ...
                dprint(infostr.format("Calibration", gglobs.calibration1st))
            ...
            if t != "WARNING":
                ...
                dprint(infostr.format("Calibration 2nd tube", gglobs.calibration2nd))

            # GMCDevice GMCvoltagebytes
            t = getConfigEntry("GMCDevice", "GMCvoltagebytes", "upper" )
            if t != "WARNING":
                if   t == '1':              gglobs.GMCvoltagebytes = 1
                elif t == '5':              gglobs.GMCvoltagebytes = 5
                else:                       gglobs.GMCvoltagebytes = 'auto'
                dprint(infostr.format("GMCvoltagebytes", gglobs.GMCvoltagebytes))

            # GMCDevice GMCendianness
            t = getConfigEntry("GMCDevice", "GMCendianness", "upper" )
            if t != "WARNING":
                if   t == 'LITTLE':         gglobs.GMCendianness = 'little'
                elif t == 'BIG':            gglobs.GMCendianness = 'big'
                else:                       gglobs.GMCendianness = 'auto'
                dprint(infostr.format("GMCendianness", gglobs.GMCendianness))

            # GMCDevice GMCnbytes
            t = getConfigEntry("GMCDevice", "GMCnbytes", "upper" )
            if t != "WARNING":
                if t == "AUTO":             gglobs.GMCnbytes = "auto"
                else:
                    nt = int(t)
                    if nt in (2, 4):        gglobs.GMCnbytes = nt
                    else:                   gglobs.GMCnbytes = 2
                dprint(infostr.format("GMCnbytes", gglobs.GMCnbytes))

            # GMCDevice variables
            t = getConfigEntry("GMCDevice", "GMCvariables", "str" )
            if t != "WARNING":
                if t.upper() == "AUTO":     gglobs.GMCvariables = "auto"
                else:                       gglobs.GMCvariables = t
                dprint(infostr.format("GMCvariables", gglobs.GMCvariables))
    ...
    # AudioCounter
        ...
        if gglobs.AudioActivation: # show the other stuff only if activated
            ...
            t = getConfigEntry("AudioCounter", "AudioCalibration", "str" )
            if t != "WARNING":
                if t.upper() == "AUTO":                 gglobs.AudioCalibration = "auto"
                else:
                    try:
                        gglobs.AudioCalibration = float(t)
                        if gglobs.AudioCalibration <= 0:    gglobs.AudioCalibration = "auto"
                    except:                             gglobs.AudioCalibration = "auto"
                dprint(infostr.format("AudioCalibration", gglobs.AudioCalibration))
    ...
    # RadMonPlus
        ...
        if gglobs.RMActivation: # show the other stuff only if activated
            ...
            if t != "WARNING":
                if t == 'AUTO':                         gglobs.RMCalibration = "auto"
                else:
                    if float(t) > 0:                    gglobs.RMCalibration = float(t)
                    else:                               gglobs.RMCalibration = "auto"
                dprint(infostr.format("RMCalibration",  gglobs.RMCalibration))
            ...
            if t != "WARNING":
                if t.upper() == "AUTO":                 gglobs.RMVariables = "auto"
                    ...
                    if gglobs.RMVariables.count("CPM") > 1 or gglobs.RMVariables.count("CPS") > 0:
                        edprint("ALERT: Improper definition of variables: ", gglobs.RMVariables)
                        edprint("ALERT: Correct in configuration file")
                        gglobs.RMVariables = "auto"
                        edprint("ALERT: RadMon variables are reset to: ", gglobs.RMVariables)
    ...
    # AmbioMon
        ...
        if gglobs.AmbioActivation: # show the other stuff only if activated
            ...
            # default in gglobs is 154 CPM / (µSv/h), (invers of 0.0065 µSv/h/CPM)
    ...
    # Gamma-Scout counter
        ...
        if gglobs.GSActivation: # show the other stuff only if activated
            ...
            t = getConfigEntry("GammaScoutDevice", "GSCalibration", "upper" )
            if t != "WARNING":
                if t == "AUTO":                         gglobs.GSCalibration = "auto"
                else:
                    try:
                        gglobs.GSCalibration = float(t)
                        if gglobs.GSCalibration <= 0:   gglobs.GSCalibration = "auto"
                    except:                             gglobs.GSCalibration = "auto"
                dprint(infostr.format("GSCalibration",  gglobs.GSCalibration))
...
###############################################################################
    ...
    # SimulCounter
        ...
        if gglobs.SimulActivation: # show the other stuff only if activated
            ...
            t = getConfigEntry("SimulCounter", "SimulPredictLimit", "int" )
            if t != "WARNING":
                try:
            ...
            t = getConfigEntry("SimulCounter", "SimulCalibration", "str" )
            if t != "WARNING":
                if t.upper() == "AUTO":             gglobs.SimulCalibration = "auto"
                else:
                    try:
                        gglobs.SimulCalibration = float(t)
                        if gglobs.SimulCalibration <= 0: gglobs.SimulCalibration = "auto"
                    except:                         gglobs.SimulCalibration = "auto"
                dprint(infostr.format("SimulCalibration", gglobs.SimulCalibration))
...
def scaleGraphValues(variable, value, scale):
    ...
    #print(fncname + "modif scale: ls: '{}'".format(ls))
    ...
    #~print("scaleGraphValues: before scaling: variable: {}, original: y-value:\n{} and more..."              .format(variable, value[:10]))
    #~print("scaleGraphValues: after scaling:  orig scale:{}, mod scale:'{}', scaled y-value:\n{} and more..." .format(scale, ls, scaledValue[:10]))
...
def colorPicker():
    ...
    if color.isValid():
        ...
        #~gglobs.varsBook[vname][4] = gglobs.varsBook[vname][4]
...
#    lp = getPortList(symlinks=True)    # get a list of all ports including symlinks
    ...
    for port in ports:
        ...
        elif device == "GS":    abr = gdev_gscout.autoBaudrateGammaScout(port)
...
def openManual():
    ...
    try:
        shortv = gglobs.__version__.split("pre")[0]   # use only the version part before '_'
        ...
        edprint("Failure Showing manual with QUrl; Exception: ", e)
        exceptPrint(e, sys.exc_info(), "openManual")
...
def showSystemInfo():
    ...
    #~from gdev_gmc import GMCmap, cfgMapKeys
    from gdev_gmc import cfgKeyHigh
    ...
    try: # the comma-separator fails if gglobs.GMC_SPIRpage is still 'auto'
        si += fmt.format("  GMC_SPIRpage Size (bytes):", "{:,}"    .format(gglobs.GMC_SPIRpage))
    except:
        si += fmt.format("  GMC_SPIRpage Size (bytes):", "{:}"    .format(gglobs.GMC_SPIRpage))

    si += fmt.format("  GMC_SPIRbugfix:",                "{:}"    .format(gglobs.GMC_SPIRbugfix))
    si += fmt.format("  GMCconfigsize (bytes):",         "{:}"    .format(gglobs.GMCconfigsize))
    si += fmt.format("  Calibration (CPM/(µSv/h)):",     str(gglobs.calibration1st))
    si += fmt.format("  Calibration 2nd (CPM/(µSv/h)):", str(gglobs.calibration2nd))
    si += fmt.format("  GMCvoltagebytes (bytes):",       "{:}"    .format(gglobs.GMCvoltagebytes))
    si += fmt.format("  GMCendianness:",                 "{:}"    .format(gglobs.GMCendianness))
    si += fmt.format("  GMCnbytes:",                     "{:}"    .format(gglobs.GMCnbytes))
    si += fmt.format("  GMClocationBug:",                "{:}"    .format(gglobs.GMClocationBug))
    ...
    si += fmt.format("  Calibration (CPM/(µSv/h)):",   str(gglobs.GSCalibration))
...
def setLoggableVariables(device, DeviceVariables):
    ...
    for i in range(0, len(DevVars)):
        DevVars[i] = DevVars[i].strip()
    gglobs.Devices[device][1] = DevVars
    wprint("setLoggableVariables: Device: ", device, ", Variables:", gglobs.Devices[device][1])


def setCalibrations(DeviceVariables, DeviceCalibration):
    """set the calibration for the defined variables"""

    if DeviceVariables == None: return  # is None for current GammaScout counter

    devvar = DeviceVariables.split(",")
    for a in devvar:
        a = a.strip()
        if len(a) == 0: continue
        if   "1st" in a: gglobs.calibration1st = DeviceCalibration   # CPM1st, CPS1st
        if   "2nd" in a: gglobs.calibration2nd = DeviceCalibration   # CPM2nd, CPS2nd
        if   "3rd" in a: gglobs.calibration3rd = DeviceCalibration   # CPM3rd, CPS3rd
        else:            gglobs.calibration1st = DeviceCalibration   # CPM, CPS, CPM1st, CPS1st

    wprint("DeviceVariables: ", DeviceVariables)
    wprint("calib 1st:",   gglobs.calibration1st)
    wprint("calib 2nd:",   gglobs.calibration2nd)
    wprint("calib 3rd:",   gglobs.calibration3rd)
...
def printLoggedValues(rfncname, varlist, alldata):
    ...
    # print like:
    # getGMC_Values:           176.00    4.00      176.00    4.00      0.00      0.00      -         -         -         -         -         -
